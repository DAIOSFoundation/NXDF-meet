...
function _mapStateToProps(state): Object {
    ...
    const dynamicBrandingImages =
        state["features/dynamic-branding"].virtualBackgrounds;
    ...
    return {
        ...
        _showUploadButton: !(
            hasBrandingImages ||
            state["features/base/config"].disableAddingBackgroundImages
        ),
...
const VirtualBackgroundDialog = translate(
    connect(_mapStateToProps)(VirtualBackground)
);
...
function VirtualBackground({
    ...
    const [storedImages, setStoredImages] = useState<Array<Image>>(
        (localImages && Bourne.parse(localImages)) || []
    );
    const [loading, setLoading] = useState(false);
    const { disableScreensharingVirtualBackground } = useSelector(
        (state) => state["features/base/config"]
    );

    const [activeDesktopVideo] = useState(
        _virtualBackground?.virtualSource?.videoType === VIDEO_TYPE.DESKTOP
            ? _virtualBackground.virtualSource
            : null
    );
    const [initialVirtualBackground] = useState(_virtualBackground);
    const deleteStoredImage = useCallback(
        (e) => {
    ...
    const deleteStoredImageKeyPress = useCallback(
        (e) => {
    ...
    const enableBlurKeyPress = useCallback(
        (e) => {
            if (e.key === " " || e.key === "Enter") {
                e.preventDefault();
                enableBlur();
    ...
    const enableSlideBlur = useCallback(async () => {
        ...
        logger.info(
            '"Slight-blur" option setted for virtual background preview!'
        );
    }, []);

    const enableSlideBlurKeyPress = useCallback(
        (e) => {
            if (e.key === " " || e.key === "Enter") {
                e.preventDefault();
                enableSlideBlur();
    ...
    const shareDesktop = useCallback(async () => {
        ...
        if (!url) {
            if (!isCancelled) {
                ...
                logger.error(
                    "Could not create desktop share as a virtual background!"
                );
            }
            ...
            if (browser.isElectron()) {
                dispatch(
                    openDialog(VirtualBackgroundDialog, {
        ...
        if (browser.isElectron()) {
            ...
            logger.info(
                '"Desktop-share" option setted for virtual background preview!'
            );
        }
    }, [dispatch, options]);

    const shareDesktopKeyPress = useCallback(
        (e) => {
            if (e.key === " " || e.key === "Enter") {
                e.preventDefault();
                shareDesktop();
    ...
    const removeBackgroundKeyPress = useCallback(
        (e) => {
            if (e.key === " " || e.key === "Enter") {
                e.preventDefault();
                removeBackground();
    ...
    const setUploadedImageBackground = useCallback(
        async (e) => {
            const imageId = e.currentTarget.getAttribute("data-imageid");
            ...
            if (image) {
                ...
                logger.info(
                    "Uploaded image setted for virtual background preview!"
                );
            }
        },
        [storedImages]
    ...
    const setImageBackgroundKeyPress = useCallback(
        (e) => {
            if (e.key === " " || e.key === "Enter") {
                e.preventDefault();
    ...
    const setUploadedImageBackgroundKeyPress = useCallback(
        (e) => {
            if (e.key === " " || e.key === "Enter") {
                e.preventDefault();
    ...
    const applyVirtualBackground = useCallback(async () => {
        ...
        if (
            _localFlipX &&
            options.backgroundType === VIRTUAL_BACKGROUND_TYPE.DESKTOP_SHARE
        ) {
            dispatch(
                updateSettings({
        ...
        logger.info(
            `Virtual background type: '${
                typeof options.backgroundType === "undefined"
                    ? "none"
                ...
                <div>
                    ...
                    <div
                        className={clsx(classes.dialog, {
                            [classes.dialogMarginTop]: previewIsLoaded,
                        })}
                        role="radiogroup"
                        tabIndex="-1"
                    >
                        ...
                        >
                            <div
                                aria-checked={
                                    _selectedThumbnail === "slight-blur"
                                }
                        ...
                        {!disableScreensharingVirtualBackground && (
                            ...
                            >
                                <div
                                    aria-checked={
                                        _selectedThumbnail === "desktop-share"
                                    }
                                    aria-label={t(
                                        "virtualBackground.desktopShare"
                                    )}
                                    className={clsx(
                                        "background-option",
                                        "desktop-share",
                        ...
                        {_images.map((image) => (
                            <Tooltip
                                content={
                                    image.tooltip &&
                                    t(`virtualBackground.${image.tooltip}`)
                                }
                                key={image.id}
                                position={"top"}
                            >
                                <img
                                    ...
                                    className={
                                        options.selectedThumbnail ===
                                            image.id ||
                                        _selectedThumbnail === image.id
                                            ? "background-option thumbnail-selected"
                                            : "background-option thumbnail"
                                    }
                        ...
                        {storedImages.map((image, index) => (
                            ...
                            >
                                <img
                                    alt={t("virtualBackground.uploadedImage", {
                                        index: index + 1,
                                    })}
                                    aria-checked={
                                        _selectedThumbnail === image.id
                                    ...
                                    onError={onError}
                                    onKeyPress={
                                        setUploadedImageBackgroundKeyPress
                                    }
                                    role="radio"
                                    src={image.src}
                                    tabIndex={0}
